...
class MeowEvaluator(object):
    ...
    def eval(self, ydf):
        ...
        spr = ydf[[self.predictionCol, self.ycol]].corr(method="spearman").to_numpy()[0, 1]
        ...
        if all(c in ydf_for_group.columns for c in ["date", "interval", self.predictionCol, self.ycol]):
            def _slice_corr(g: pd.DataFrame, method: str = "pearson"):
                if g.shape[0] < 2:
                    return np.nan
                return g.corr(method=method).to_numpy()[0, 1]

            slice_df = ydf_for_group[["date", "interval", self.predictionCol, self.ycol]]
            gb = slice_df.groupby(["date", "interval"], sort=False)[[self.predictionCol, self.ycol]]
        ...
        if "symbol" in ydf_for_group.columns and all(
            c in ydf_for_group.columns for c in [self.predictionCol, self.ycol]
        ):
            sym_gb = ydf_for_group.groupby("symbol", sort=False)[[self.predictionCol, self.ycol]]
